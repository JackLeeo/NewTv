...
//   1. **Background Audio (方案 A)**: AppDelegate 启动 AVAudioSession 持续
//      播放静音 buffer, iOS 把 app 当作 "audio app", 维持 app 在后台 /
//      锁屏时活跃, **不会 SIGKILL embed library**. Node.js 进程持续运行,
//      切回前台时 listen socket 还在, verify 1-5ms 通过.
//   2. **appDidBecomeActive ping**: 兜底, 监听 UIApplication.didBecomeActiveNotification,
//      每次 app 回前台 ping http://127.0.0.1:managementPort/check, 失败 →
//      isRunning=false + notifyDart('onNodeExit'). Dart 收到后清状态, 调
//      startNodeJS 重启.
//
// **目录约定**：
...
//
// **注意**：
//   - NodeMobile.h 只导出 `int node_start(int argc, char* argv[])`
//   - stopNodeJS 只能重置内部状态；Node.js 进程要自然退出或 iOS SIGKILL
...
@objc class NodeJSBridge: NSObject {
    ...
    private func stopNodeJS() {
        ...
        //
        // **2026-07-08 撤销方案 B dlsym 降级**:
        //   方案 A (Background Audio) 单独 work 验证通过 (1h30m+ Node.js 持续运行
        //   + 4分17秒锁屏后 verify 1ms 通过). 不再需要 dlsym 查 node_exit
        //   符号. stopNodeJS 回到最简实现.
        if isRunning {
            print("[NodeJSBridge] stopNodeJS: reset 内部状态 (Node.js 进程继续跑, Background Audio 防 SIGKILL)")
            isRunning = false
            managementPort = 0
        }
    }
